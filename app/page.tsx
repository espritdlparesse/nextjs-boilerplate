"use client";
import type { LibraryView, Tab } from "@/app/types";
...
export default function Page() {
  ...
  const addForm = useAddForm({ loadLibrary });
  const share = useShareCard({ items, vibe });
  const imports = useImports({ loadLibrary, setTab });
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {/* ABOUT / ONBOARDING */}
        {tab === "home" && <HomeTab setTab={setTab} aboutStep={aboutStep} setAboutStep={setAboutStep} />}

        {tab === "profile" && <ProfileTab tgUserId={tgUserId} counts={counts} countsUnknown={countsUnknown} headerAvatar={headerAvatar} adminViewOff={adminViewOff} toggleAdminView={toggleAdminView} importServices={importServices} setTab={setTab} imports={imports} profile={profile} />}

        {/* ADD */}
        {tab === "add" && <AddTab importServices={importServices} imports={imports} deepVibe={deepVibe} addForm={addForm} />}

        {/* LIBRARY */}
        {tab === "library" && <LibraryTab items={items} libraryLoading={libraryLoading} libraryError={libraryError} libraryView={libraryView} setLibraryView={setLibraryView} setTab={setTab} customCategories={addForm.customCategories} deletingId={library.deletingId} deleteItem={library.deleteItem} library={library} />}

        {/* VIBE */}
        {tab === "vibe" && <VibeTab counts={counts} countsUnknown={countsUnknown} shareVibeCard={share.shareVibeCard} vibe={vibe} deepVibe={deepVibe} />}
