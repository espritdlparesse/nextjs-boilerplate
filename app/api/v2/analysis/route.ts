...
export async function POST(req: NextRequest) {
  ...
  const baseQuery = sb.from("items").select("type, title, creator").or(buildOwnerReadFilter(scope));
    ...
    messages: [
      {
        role: "system",
        content:
          "Ты пишешь короткий, умный и наблюдательный культурный срез вкуса. Верни только JSON без markdown с полями summary:string и highlights:string[]. summary — 2-4 предложения по-русски, с конкретными наблюдениями про вкус. highlights — 4-6 коротких пунктов: паттерны, противоречия, рекомендации, красные флаги вкуса. Тон: остро, смешно, наблюдательно, но не хамски и не токсично.",
      },
      {
        role: "user",
        content: `Сделай культурный срез по этому списку контента и верни JSON.\n\n${lines.slice(0, 120).join("\n")}`,
      },
    ],
  });
