...
`joinRoastLines` ставит точку в конце каждой строки перед склейкой, поэтому `looksTooComplicated` мерит хук и вывод по отдельности. До 2026-09-20 хук приходил без точки, склеенная фраза набирала 23–27 слов, и порог в 22 слова отклонял почти каждый прогон. Порог считает только слова: тире и другие одиночные знаки в счёт не идут.
...
Замер на 2026-09-06: из 431 карточки фильтр пропускает 251. Отсеяны 117 пустой похвалы, 89 новостей, 4 канцелярита. Старые карточки не удаляются — они просто не доходят до модели.
